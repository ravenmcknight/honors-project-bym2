...
apc <- readRDS('data/covid/raw/apc-interpolated.RDS')
setDT(apc)
...
saveRDS("data/covid/apc_loc.RDS")
...
saveRDS(apc_annual_bg, 'data/covid/aggregations/apc_annual_bg.RDS')
...
saveRDS(apc_annual_bg, 'data/covid/aggregations/apc_annual_stop.RDS')
...
saveRDS(apc_daily, 'data/covid/aggregations/apc_daily.RDS')
...
saveRDS(apc_daily_bg, 'data/covid/aggregations/apc_daily_bg.RDS')
...
saveRDS(apc_avg_daily_bg, 'data/covid/aggregations/apc_avg_daily_bg.RDS')
...
saveRDS(apc_daily_stop, 'data/covid/aggregations/apc_daily_stop.RDS')

# slightly more practical average
apc_avg_daily_stop <- apc_daily_stop[, .(daily_boards = mean(daily_boards, na.rm = T), daily_alights = mean(daily_alights, na.rm = T), 
                                     num_interpolated = mean(num_interpolated, na.rm = T), num_routes = mean(num_routes, na.rm = T), 
                                     daily_stops = mean(daily_stops, na.rm = T)), keyby = .(site_id)]

saveRDS(apc_avg_daily_stop, 'data/covid/aggregations/apc_avg_daily_stop.RDS')
