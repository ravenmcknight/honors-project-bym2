...
Often, the coefficients $\beta_1, ..., \beta_k$ are given simple normal priors such as $\beta_k \sim$ Normal(0, 1) in the case of standardized parameters. When parameter vector $x$ contains many possible covariates, we may reasonably assume that some entries in $\beta$ are 0 -- in other words, that some of the parameters in $x$ have no effect on $Y$. In such a case, it is beneficial to use *sparsity inducing priors* on $\beta$ to ensure that irrelevant parameter coefficients are shrunk to 0. 

In frequentist applications, methods such as the LASSO (Least Absolute Shrinkage and Selection Operator) are employed for this type of variable selection. In the Bayesian setting, shrinkage and regularization can be performed using a variety of different priors on $\beta$. **Cite van Erp etc** provide an introduction to many such continuous priors. Ridge regression and the Bayesian LASSO are two of the most approchable sparse regression priors. The horseshoe prior, proposed by **Carvalho et al., 2010**, is another popular choice in Bayesian literature **()** (somehow this accidentally came out sounding exactly like van Erp). Following the notation of **Vehtari**, the horsehoe prior can be written
...
where $c$ > 0 helps to regularize $\beta_k$ far from zero. When $c$ approaches infinity, the regularized horseshoe becomes the standard horseshoe. **This is a place where my understanding is weak**
