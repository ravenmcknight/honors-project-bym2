...
y <- mod_dat$daily_boards_per_stop
...
poisson <- "/Users/mcknigri/Documents/honors/honors-project/stan/poisson.stan"
...
yrep <- yrep[sample(nrow(yrep), 100), ]

pp_check(y, yrep, ppc_dens_overlay) + 
  theme_minimal() +
  xlim(0, 200) +
  labs(title = "Actual versus fitted") + xlim(-1, 5)
```
...
poisson_theta_fit <- stan(poisson_theta, data = standat1, warmup = 1000, iter = 2000)
...
theta_yrep <- fit$y_rep
theta_yrep <- theta_yrep[sample(nrow(theta_yrep), 100), ]

pp_check(y, theta_yrep, ppc_dens_overlay) + 
  theme_minimal() +
  xlim(0, 200) +
  labs(title = "Actual versus fitted") + xlim(-1, 5)
```
